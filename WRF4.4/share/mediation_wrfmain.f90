...
SUBROUTINE med_add_config_info_to_grid ( grid )
 ...
 grid % gsfcrad_gocart_coupling    = model_config_rec % gsfcrad_gocart_coupling 
 grid % gsfcgce_gocart_coupling    = model_config_rec % gsfcgce_gocart_coupling 
 grid % auxinput12_inname          = model_config_rec % auxinput12_inname 
 grid % io_form_auxinput12         = model_config_rec % io_form_auxinput12 
 grid % emi_inname                 = model_config_rec % emi_inname 
 grid % fireemi_inname             = model_config_rec % fireemi_inname 
 grid % input_chem_inname          = model_config_rec % input_chem_inname 
 grid % emi_outname                = model_config_rec % emi_outname 
 grid % fireemi_outname            = model_config_rec % fireemi_outname 
 grid % input_chem_outname         = model_config_rec % input_chem_outname 
 grid % io_style_emissions         = model_config_rec % io_style_emissions 
 grid % bioemdt                    = model_config_rec % bioemdt (grid%id)
 grid % photdt                     = model_config_rec % photdt (grid%id)
 grid % chemdt                     = model_config_rec % chemdt (grid%id)
 grid % ne_area                    = model_config_rec % ne_area 
 grid % kemit                      = model_config_rec % kemit 
 grid % nmegan                     = model_config_rec % nmegan 
 grid % kfuture                    = model_config_rec % kfuture 
 grid % kfire                      = model_config_rec % kfire 
 grid % kemit_aircraft             = model_config_rec % kemit_aircraft 
 grid % kdvel                      = model_config_rec % kdvel 
 grid % ndepvel                    = model_config_rec % ndepvel 
 grid % kdepvel                    = model_config_rec % kdepvel 
 grid % biomass_emiss_opt          = model_config_rec % biomass_emiss_opt (grid%id)
 grid % cam_mam_mode               = model_config_rec % cam_mam_mode 
 grid % cam_mam_nspec              = model_config_rec % cam_mam_nspec 
 grid % cam_mp_mam_cpled           = model_config_rec % cam_mp_mam_cpled 
 grid % mozart_ph_diag             = model_config_rec % mozart_ph_diag 
 grid % megan_specifier            = model_config_rec % megan_specifier (grid%id)
 grid % megan_factors_file         = model_config_rec % megan_factors_file 
 grid % megan_mapped_emisfctrs     = model_config_rec % megan_mapped_emisfctrs 
 grid % vprm_opt                   = model_config_rec % vprm_opt (grid%id)
 grid % wpeat                      = model_config_rec % wpeat 
 grid % wflood                     = model_config_rec % wflood 
 grid % term_opt                   = model_config_rec % term_opt (grid%id)
 grid % do_pvozone                 = model_config_rec % do_pvozone 
 grid % chem_conv_tr               = model_config_rec % chem_conv_tr (grid%id)
 grid % conv_tr_wetscav            = model_config_rec % conv_tr_wetscav (grid%id)
 grid % conv_tr_aqchem             = model_config_rec % conv_tr_aqchem (grid%id)
 grid % chem_opt                   = model_config_rec % chem_opt (grid%id)
 grid % gaschem_onoff              = model_config_rec % gaschem_onoff (grid%id)
 grid % aerchem_onoff              = model_config_rec % aerchem_onoff (grid%id)
 grid % wetscav_onoff              = model_config_rec % wetscav_onoff (grid%id)
 grid % dustwd_onoff               = model_config_rec % dustwd_onoff (grid%id)
 grid % cldchem_onoff              = model_config_rec % cldchem_onoff (grid%id)
 grid % is_full_tuv                = model_config_rec % is_full_tuv (grid%id)
 grid % lambda_cutoff              = model_config_rec % lambda_cutoff (grid%id)
 grid % cld_od_opt                 = model_config_rec % cld_od_opt (grid%id)
 grid % pht_cldfrc_opt             = model_config_rec % pht_cldfrc_opt (grid%id)
 grid % phot_blcld                 = model_config_rec % phot_blcld (grid%id)
 grid % vertmix_onoff              = model_config_rec % vertmix_onoff (grid%id)
 grid % chem_in_opt                = model_config_rec % chem_in_opt (grid%id)
 grid % phot_opt                   = model_config_rec % phot_opt (grid%id)
 grid % gas_drydep_opt             = model_config_rec % gas_drydep_opt (grid%id)
 grid % aer_drydep_opt             = model_config_rec % aer_drydep_opt (grid%id)
 grid % diagnostic_chem            = model_config_rec % diagnostic_chem (grid%id)
 grid % aero_diag_opt              = model_config_rec % aero_diag_opt (grid%id)
 grid % aero_cw_diag_opt           = model_config_rec % aero_cw_diag_opt (grid%id)
 grid % kfcup_diag                 = model_config_rec % kfcup_diag (grid%id)
 grid % aer_aerodynres_opt         = model_config_rec % aer_aerodynres_opt (grid%id)
 grid % emiss_opt                  = model_config_rec % emiss_opt (grid%id)
 grid % emiss_opt_vol              = model_config_rec % emiss_opt_vol (grid%id)
 grid % dust_opt                   = model_config_rec % dust_opt 
 grid % dust_schme                 = model_config_rec % dust_schme 
 grid % dmsemis_opt                = model_config_rec % dmsemis_opt 
 grid % seas_opt                   = model_config_rec % seas_opt 
 grid % bio_emiss_opt              = model_config_rec % bio_emiss_opt (grid%id)
 grid % biomass_burn_opt           = model_config_rec % biomass_burn_opt (grid%id)
 grid % plumerisefire_frq          = model_config_rec % plumerisefire_frq (grid%id)
 grid % emiss_inpt_opt             = model_config_rec % emiss_inpt_opt (grid%id)
 grid % gas_bc_opt                 = model_config_rec % gas_bc_opt (grid%id)
 grid % gas_ic_opt                 = model_config_rec % gas_ic_opt (grid%id)
 grid % aer_bc_opt                 = model_config_rec % aer_bc_opt (grid%id)
 grid % aer_ic_opt                 = model_config_rec % aer_ic_opt (grid%id)
 grid % have_bcs_chem              = model_config_rec % have_bcs_chem (grid%id)
 grid % have_bcs_tracer            = model_config_rec % have_bcs_tracer (grid%id)
 grid % scale_fire_emiss           = model_config_rec % scale_fire_emiss (grid%id)
 grid % aer_ra_feedback            = model_config_rec % aer_ra_feedback (grid%id)
 grid % aer_op_opt                 = model_config_rec % aer_op_opt (grid%id)
 grid % opt_pars_out               = model_config_rec % opt_pars_out 
 grid % diagnostic_dep             = model_config_rec % diagnostic_dep (grid%id)
 grid % aircraft_emiss_opt         = model_config_rec % aircraft_emiss_opt (grid%id)
 grid % have_bcs_upper             = model_config_rec % have_bcs_upper (grid%id)
 grid % fixed_ubc_press            = model_config_rec % fixed_ubc_press (grid%id)
 grid % fixed_ubc_inname           = model_config_rec % fixed_ubc_inname 
 grid % trop_lev_inname            = model_config_rec % trop_lev_inname 
 grid % exo_coldens_inname         = model_config_rec % exo_coldens_inname (grid%id)
 grid % wes_seasonal_inname        = model_config_rec % wes_seasonal_inname (grid%id)
 grid % chemdiag                   = model_config_rec % chemdiag (grid%id)
 grid % aero_srf_area_diag         = model_config_rec % aero_srf_area_diag (grid%id)
 grid % dust_alpha                 = model_config_rec % dust_alpha 
 grid % dust_gamma                 = model_config_rec % dust_gamma 
 grid % dust_smtune                = model_config_rec % dust_smtune 
 grid % dust_ustune                = model_config_rec % dust_ustune 
 grid % dust_dsr                   = model_config_rec % dust_dsr 
 grid % dust_veg                   = model_config_rec % dust_veg 
 grid % dust_soils                 = model_config_rec % dust_soils 
 grid % dust_smois                 = model_config_rec % dust_smois 
 grid % emiss_ash_hgt              = model_config_rec % emiss_ash_hgt 
 grid % depo_fact                  = model_config_rec % depo_fact (grid%id)
 grid % track_chem_num             = model_config_rec % track_chem_num 
 grid % track_chem_name            = model_config_rec % track_chem_name (grid%id)
 grid % track_rad_num              = model_config_rec % track_rad_num 
 grid % track_tuv_num              = model_config_rec % track_tuv_num 
 grid % track_tuv_lev              = model_config_rec % track_tuv_lev 
 grid % n2o5_hetchem               = model_config_rec % n2o5_hetchem 
 grid % mosaic_aerchem_optaa       = model_config_rec % mosaic_aerchem_optaa 
 grid % af_lambda_start            = model_config_rec % af_lambda_start (grid%id)
 grid % af_lambda_end              = model_config_rec % af_lambda_end (grid%id)
 grid % do_isorropia               = model_config_rec % do_isorropia 
 grid % n_ic                       = model_config_rec % n_ic (grid%id)
 grid % n_cg                       = model_config_rec % n_cg (grid%id)
 grid % lnox_opt                   = model_config_rec % lnox_opt (grid%id)
 grid % lnox_passive               = model_config_rec % lnox_passive (grid%id)
 grid % ltng_temp_upper            = model_config_rec % ltng_temp_upper (grid%id)
 grid % ltng_temp_lower            = model_config_rec % ltng_temp_lower (grid%id)
 grid % has_o3_exo_coldens         = model_config_rec % has_o3_exo_coldens 
 grid % du_at_grnd                 = model_config_rec % du_at_grnd 
 grid % scale_o3_to_grnd_exo_coldens = model_config_rec % scale_o3_to_grnd_exo_coldens 
 grid % scale_o3_to_du_at_grnd     = model_config_rec % scale_o3_to_du_at_grnd 
 grid % irr_opt                    = model_config_rec % irr_opt (grid%id)
 grid % num_gca_levels             = model_config_rec % num_gca_levels 
 grid % gca_input_opt              = model_config_rec % gca_input_opt 
