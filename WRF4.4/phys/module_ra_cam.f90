...
subroutine camrad(RTHRATENLW,RTHRATENSW,RTHRATENLWC,RTHRATENSWC,   &
  ...
  real, dimension(ims:ime,jms:jme), optional, intent(in) :: coszen
   ...
   real(r8), dimension(  1:ite-its+1, 1:kte-kts+1 ) :: qrscs    
   ...
   real(r8), dimension(  1:ite-its+1, 1:kte-kts+1 ) :: qrlcs    
...
'CLWRF-module_ra_cam. : AEROSOLt=NaN')
               call endrun ()
            end if
         end do
      end do
...
subroutine radctl(j, lchnk   ,ncol    , pcols, pver, pverp, pverr, pverrp, ppcnst, pcnst,  &
   ...
   real(r8), intent(out) :: qrscs(pcols,pver)    
   ...
   real(r8), intent(out) :: qrlcs(pcols,pver)    
