...
use module_wrf_error
   ...
   REAL, ALLOCATABLE, DIMENSION(:) :: PV_FRAC_ROOF_TBL 
   REAL, ALLOCATABLE, DIMENSION(:) :: GR_FRAC_ROOF_TBL 
   INTEGER :: GR_FLAG_TBL 
   INTEGER :: GR_TYPE_TBL 
...
'Error opening URBPARM.TBL. Make sure URBPARM.TBL (found in run/) is linked to the running directory.' )
         ENDIF
...
'Error opening URBPARM_LCZ.TBL. Make sure URBPARM_LCZ.TBL (found in run/) is linked to the running directory.' )
         ENDIF
 endif
...
'Error allocating Z0B_TBL in urban_param_init' )
            ...
            if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2124,&
...
'Error allocating GAPTEMP_TBL in urban_param_init' )
            ...
            if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2204,&
...
'Error allocating GAPHUM_TBL in urban_param_init' )
            ...
            if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2210,&
'Error allocating PERFLO_TBL in urban_param_init' )
            ALLOCATE( HSESF_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2213,&
'Error allocating HSESF_TBL in urban_param_init' )
            ALLOCATE( PV_FRAC_ROOF_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) CALL wrf_error_fatal3("<stdin>",2216,&
'Error allocating PV_FRAC_ROOF_TBL in urban_param_init')
            ALLOCATE( GR_FRAC_ROOF_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) CALL wrf_error_fatal3("<stdin>",2219,&
'Error allocating GR_FRAC_ROOF_TBL in urban_param_init')
         ...
         if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2238,&
'Error allocating ROOF_WIDTH in urban_param_init' )
         ...
         if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2244,&
...
'URBPARM.TBL: Unrecognized NAME = "'//trim(name)//'" in Subr URBAN_PARAM_INIT' )
   ...
   REAL, DIMENSION( ims:ime, 1:urban_map_zdf, jms:jme ),INTENT(INOUT) :: DGR_URB3D 
   REAL, DIMENSION( ims:ime, 1:num_urban_ndm, jms:jme ),INTENT(INOUT) :: DG_URB3D 
   REAL, DIMENSION( ims:ime, 1:urban_map_zdf, jms:jme ),INTENT(INOUT) :: LFR_URB3D 
   REAL, DIMENSION( ims:ime, 1:num_urban_ndm, jms:jme ), INTENT(INOUT) :: LFG_URB3D 
   ...
   DO I=ims,ime
    DO J=jms,jme
      ...
      if (SF_URBAN_PHYSICS.EQ.3) then
       ...
       ELSE
       TRV_URB4D(I,:,J)=tlayer0_urb(I,1,J) 
      ENDIF
