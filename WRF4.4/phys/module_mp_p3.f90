 ...
 integer, parameter :: zsize        = 11  
 integer, parameter :: densize      =  5
 integer, parameter :: rimsize      =  4
 integer, parameter :: rcollsize    = 30
 integer, parameter :: tabsize      = 14  
 integer, parameter :: tabsize_3mom = 15  
 ...
 real, dimension(zsize,densize,rimsize,isize,tabsize_3mom)          :: itab_3mom   
 ...
 character(len=*), intent(in)             :: lookup_file_dir    
 integer,          intent(in)             :: nCat               
 logical,          intent(in)             :: trplMomI           
 ...
 character(len=1024), parameter :: version_intended_table_1_2mom = '5.3-2momI'
 character(len=1024), parameter :: version_intended_table_1_3mom = '3momI_v5.1.7'
 character(len=1024), parameter :: version_intended_table_2      = '2momI_v5.2.3'
...
END subroutine p3_init
 ...
 real, intent(inout), dimension(ni,nk), optional  :: zitot_1     
 ...
 real, intent(inout), dimension(ni,nk), optional  :: zitot_2     
 ...
 real, intent(inout), dimension(ni,nk), optional  :: zitot_3     
 ...
 real, intent(inout), dimension(ni,nk), optional  :: zitot_4     

 real, intent(in), dimension(ni,nk)  :: qvap_m                
 real, intent(inout), dimension(ni,nk)  :: qvap                  
 real, intent(in), dimension(ni,nk)  :: temp_m                
 ...
 real, dimension(ni,nk,n_iceCat)  :: zitot      
 ...
 real, dimension(ni,nk)  :: qqdelta,ttdelta     
 ...
 real, intent(out),   dimension(its:ite,kts:kte,nCat), optional :: diag_dhmax 
 real, intent(out),   dimension(its:ite,kts:kte,nCat), optional :: diag_lami  
 real, intent(out),   dimension(its:ite,kts:kte,nCat), optional :: diag_mui   
 ...
 real, intent(inout), dimension(its:ite,kts:kte,nCat),   optional :: zitot   
...
444    continue
       ...
       if (.not.(log_predictNc).and.sup_cld.gt.1.e-6.and.it.gt.1) then
          ...
          ab    = 1. + dqsdT*xxlv(i,k)*inv_cp
          dum   = max(0.,min(dum,(Qv_cld(k)-dumqvs)/ab))  
          qcnuc = dum*odt*SCF(k)
       endif
       ...
       if (qr(i,k).ge.qsmall) then
          ...
          elseif (iparam.eq.4) then
	     nrslf = dum*205.*(qr(i,k)*iSPF(k))**1.55*(nr(i,k)*1.e-6*rho(i,k)*iSPF(k))**0.6*1.e6/rho(i,k)*SPF(k) 
          endif
       ...
       qv_tmp = Qv_cld(k) + (-qcnuc-qcacc-qrcon+qcevp+qrevp)*dt                              
       t_tmp  = t(i,k) + (qcnuc+qccon+qrcon-qcevp-qrevp)*xxlv(i,k)*inv_cp*dt                 
      ...
      update_refl_processes: if (log_3momentIce) then
       ...
       iice_loop_z: do iice = 1,nCat
          ...
          tmp2 =  nrhetc(iice) + nrheti(iice)                   
          if (tmp2.ge.qsmall) then
             tmp1 = (qrhetc(iice) + qrheti(iice))*6./(900.*pi)  
             mu_i_new = mu_r(i,k)
             zitot(i,k,iice) = zitot(i,k,iice) + G_of_mu(mu_i_new)*tmp1**2/tmp2*dt
          endif

        
          tmp2 =  nchetc(iice) + ncheti(iice)                   
          if (tmp2.ge.qsmall) then
             tmp1 = (qchetc(iice) + qcheti(iice))*6./(900.*pi)  
             mu_i_new = mu_c(i,k)
             zitot(i,k,iice) = zitot(i,k,iice) + G_of_mu(mu_i_new)*tmp1**2/tmp2*dt
          endif
          ...
          tmp2 = ninuc(iice)                                    
          if (tmp2.ge.qsmall) then
             tmp1 = qinuc(iice)*6./(900.*pi)                    
             mu_i_new = mu_i_initial                            
             zitot(i,k,iice) = zitot(i,k,iice) + G_of_mu(mu_i_new)*tmp1**2/tmp2*dt
          endif
       ...
       iice_loop2: do iice = 1,nCat
          ...
          if (qitot(i,k,iice).ge.qsmall .and. birim(i,k,iice).ge.bsmall .and. qimlt(iice)>0.) then
             tmp1 = qirim(i,k,iice)/birim(i,k,iice)     
             tmp2 = qitot(i,k,iice) + qimlt(iice)*dt    
    ...
    i_loop_typediag: do i = its,ite
       ...
       k_loop_typdiag_1: do k = kbot,ktop_typeDiag,kdir
          ...
          if (qr(i,k)>qsmall .and. nr(i,k)>nsmall) then
             tmp1 = (6.*qr(i,k)/(pi*rhow*nr(i,k)))**thrd   
...
SUBROUTINE access_lookup_table_coll_3mom(dumzz,dumjj,dumii,dumj,dumi,index,dum1,dum3,          &
 ...
 real,    intent(in)  :: qitot,nitot,qirim,rhop
             ...
             dum1 = (alog10(qitot/nitot)+18.)*3.444606 - 10.             
 ...
 real                 :: drhop
                      ...
                      dum1 = (alog10(qitot_1/nitot_1)+18.)*1.722303 - 5.             
                      ...
                      dum1c = (alog10(qitot_2/nitot_2)+18.)*1.722303 - 5.             
 ...
 subroutine check_values(Qv,T,Qc,Nc,Qr,Nr,Qitot,Qirim,Nitot,Birim,i,timestepcount,       &
  ...
  logical,              intent(in) :: force_abort_in         
     ...
     do iice = 1,ncat
        ...
        if (present(Zitot)) then
           if ( .not.(Qitot(k,iice)==0. .and. Nitot(k,iice)==0. .and. Zitot(k,iice)==0.) .and.                &
               ( Qitot(k,iice)>0. .and. Nitot(k,iice)>0. .and. Zitot(k,iice)<=0. )                            &  
               .and. source_ind /= 100                                                                        &  
               .and. source_ind /= 200                                                                        &  
               .and. source_ind /= 300 ) then                                                                    
 ...
 real, parameter  :: dD       =   1.e-3  
 real, parameter  :: Dmax_psd = 200.e-3  
 real, parameter  :: FrThrs   = 0.75     

 real, parameter  :: Rcrit    = 1.e-3/6. 
 real, parameter  :: ch       = 206.89   
 real, parameter  :: dh       = 0.6384   
 real             :: n0                  
 real             :: Frim                
 real             :: Di                  
 real             :: N_tot               
