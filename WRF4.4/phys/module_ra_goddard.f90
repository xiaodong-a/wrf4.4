...
data ((mcdat(ilev,10,ifld),ifld=1,6),ilev=12,22)/  &
 ...
 if( .not. allocated(kext_q_unit_array) ) then
   ...
   allocate( kext_q_unit_array ( mxpts_re_visir, id_hmax, mxband, mxisl ), &
             ...
             stat=ierr )
   if (ierr /= 0) call wrf_error_fatal3("<stdin>",820,&
'MSG read_lut_nc: LUT parameter allocation error -> Terminate program. ')
 ...
 if( .not. allocated(kext_sw_cloud) ) then
     ...
     stat=ierr )
     if (ierr /= 0) call wrf_error_fatal3("<stdin>",849,&
'MSG read_lut_nc: LUT parameter allocation error -> Terminate program. ')
 ...
 real, dimension( ims:ime, jms:jme, 1:4 ), intent(out),optional  :: sflxd    
  ...
  real, dimension(ims:ime, jms:jme),intent(out),optional :: aod2d_out        
                                                                             
  real, dimension(ims:ime,jms:jme),intent(out),optional :: atop2d_out        
...
real, dimension( ims:ime, jms:jme ), intent(in)  :: &
 ...
 real, dimension( CHUNK, kts:kte, num_go) :: aero1d  
    ...
    ENDDO
     ...
     if ( (chem_opt == 300 .or. chem_opt == 301 .or. &
        ...
        if( radiation_skip ) then
         DO ic=1,min(CHUNK,ite-ii+1)
          i = ii+ic-1
          aod2d_out(i:ie(ic),j:je(ic))  = sum(taual_sw(ic,:,8))   
         ENDDO
        else
         DO ic=1,min(CHUNK,ite-ii+1)
          i = ii+ic-1
          aod2d_out(i,j)  = sum(taual_sw(ic,:,8))   
         ENDDO
        endif
        ...
        DO ic=1,min(CHUNK,ite-ii+1)
         ...
         atop_loop: do k = kts,kte
          if( taual_sw(ic,k,8) >= 0.01 ) then
            if( radiation_skip ) then
              atop2d_out(i:ie(ic),j:je(ic)) = p1d(ic,k) 
            else
              atop2d_out(i,j) = p1d(ic,k)       
            endif
            exit atop_loop
          endif
         enddo atop_loop
...
'MSG Goddard Radiation Subroutine sounding; reduce the number of alev_strat')
 ...
 real,intent(in) :: aero(tgmx)  
 real, intent(in)    :: pr        
 real, intent(in)    :: ta        
 real, intent(in)    :: sh        
 type ( particle_gocart ),dimension(nband),intent(out) :: & 
     kext_gocart,  & 
     salb_gocart,  & 
     asym_gocart     

 type ( particle_gocart ) :: q   
 real :: rh 
 integer :: rhi         
 real :: es            
 real :: eq             
 ...
 es = 0.611 * exp( 5423.*( 1./273.15 - 1./ta ) )  
 eq = sh/(1.-sh)                                   
 e = (eq / (eq+0.622)) * pr * 0.1                   
 rh = min( max(1e-5, e/es) , 0.99)                
 ...
 real,intent(in) :: dz_km  
 integer,intent(in) :: nband  
 type ( particle_gocart ),dimension(nband),intent(in) :: & 
     kext_gocart,  & 
     salb_gocart,  & 
     asym_gocart     

 real,dimension(nband),intent(out) :: tautot, salbtot, asymtot 
