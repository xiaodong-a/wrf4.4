...
CONTAINS
 ...
 REAL, OPTIONAL, DIMENSION( ims:ime, kms:kme, jms:jme, num_chem), &
         INTENT(IN)               ::  chem
 real, dimension(ims:ime, kms:kme, jms:jme), optional, intent(out) :: aod_out  
      ...
      do j = jts, jte
         do k = kts, kte  
            do i = its, ite
               ...
               aero(i,k,j, 1) = max(0.0, chem(i,k,j,p_sulf)*1.0e-6*p(i,k,j)*96.0/(8.314*t(i,k,j)))  
     ...
     swrad_select: SELECT CASE(sw_physics)
        ...
        case (goddardswscheme)
             ...
             CALL goddardrad(sw_or_lw='sw',dx=dx                  &
                    ...
                    ,obscur=obscur_loc                                 & 
                    ,itimestep=itimestep, dt_in = dt                   &
                    ,aod2d_out=aod2d_out, atop2d_out=atop2d_out        & 
                    ,AERO=aero                                         &
                    ,CHEM_OPT=chem_opt                                 &
                    ,GSFCRAD_GOCART_COUPLING=gsfcrad_gocart_coupling   &
                                                                       )
