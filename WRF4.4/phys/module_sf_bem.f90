MODULE module_sf_bem
	...
	htot(2)=hswalins(5,1)+hrwalins(5,1)   
	 ...
	 call fluxeqocc(nhourday,bw,bl,perflo,hsesf,hsequip,hseqocc(ilev),hleqocc(ilev))
      ...
      real,intent(in),dimension(1:nz) :: tair  
      integer,intent(in) :: n  
      ...
      real,intent(inout) :: hspv 
      real,intent(inout) :: eppv 
      real,intent(inout) :: tpv       



      real,parameter :: albpv=0.11 
      real,parameter :: empv_down=0.95  
...
'singular matrix in gaussjbem')
         ...
         if(a(icol,icol).eq.0) CALL wrf_error_fatal3("<stdin>",2484,&
'singular matrix in gaussjbem')
