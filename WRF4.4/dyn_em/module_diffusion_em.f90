    ...
    USE module_state_description, only: p_m23, p_m13, p_m22, p_m33, p_r23, p_r13, p_r12, p_m12, p_m11, &
                                        P_QNS, P_QNR, P_QNG, P_QT, P_QNH, P_QVOLG 
...
END SUBROUTINE smag2d_km
   ...
   :: dlk
    ...
    REAL, PARAMETER :: xkmvo = 0.0, xkhvo = 0.0 
    ...
    IF ( config_flags%km_opt .EQ. 2 .and. isotropic .EQ. 0 ) THEN  
      DO j = j_start, j_end
      DO k = kts, ktf
      DO i = i_start, i_end
        mlen_h = SQRT( dx/msftx(i,j) * dy/msfty(i,j) )
    ...
    IF ( config_flags%km_opt .eq.2.and.isotropic .NE. 0 ) THEN  
    ...
    IF(config_flags%km_opt .eq.5 ) THEN  
       ...
       DO i = i_start, i_end
          ...
          END IF
              ...
              xkmv_meso(i,k,j) =  0.4 * tmp * dlk(i,k,j)      
              xkmv_meso(i,k,j) =  xkmv_meso(i,k,j) + xkmvo
              xkmv_les         =  c_k * tmp * mlen_v          
